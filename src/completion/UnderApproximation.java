...
public class UnderApproximation{
    ...
    private UnderApproximation previousStructure;
    private Vector<UnderApproximation> nextStructure;
    public UnderApproximation(Data data) throws CloneNotSupportedException {
        this.data = data;
        this.proc = data.getSequence().get(0);
        sol = new Vector<>();
        findSol(proc,new Solution(), new Vector<Process>());
    }
    public UnderApproximation(Data data, Process proc) throws CloneNotSupportedException {
        this.data = data;
        this.proc = proc;
        sol = new Vector<>();
        findSol(proc,new Solution(), new Vector<Process>());
    ...
    public boolean ReachSol(Solution s,Vector<Process> antiLoop) throws CloneNotSupportedException{
        for (Process p : s.getProc()){
            ...
            if (!findProcess(p)){
                ...
                UnderApproximation str =  new UnderApproximation(data, p);
